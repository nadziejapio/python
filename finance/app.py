...
def index():
    ...
    stocks = db.execute("SELECT symbol, COUNT(number), username, price, SUM(price), nazwa FROM transactions WHERE username = ? GROUP BY symbol, nazwa", db.execute("SELECT username FROM users WHERE id = ?", session["user_id"])[0].get("username"))
    for stock in stocks:
        stock.update({"pricenow": lookup(stock.symbol).get("price")})

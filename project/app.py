...
@login_required
def index():
    books = db.execute("SELECT title, status, name, surname, nick, time FROM book, person, users WHERE id = ? ", db.execute("SELECT username FROM users WHERE id = ?", session["user_id"])[0].get("username"))
    print(books)

...
def index():
    books = db.execute(
        "SELECT title, status, name, surname, nick, time FROM book, person WHERE username = ? ", db.execute("SELECT username FROM users WHERE id = ?", session["user_id"])[0].get("username"))
    c = 0
    print(stocks)
    for stock in stocks:
